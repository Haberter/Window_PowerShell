...
- **额度查询**：单 key / 所有 key 的详细使用情况（状态、每日消费、Token 统计、模型消费明细）
- **排名面板**：按剩余每日额度排序展示所有 key
- **自动切换**：当前 key 余量不足阈值时，切换到余量最多的 key
- **持续监控**：定时检测，支持前台 / 后台模式
- **GUI 选择**：后台模式下触发切换时弹出深色主题窗口供选择
...
| 命令 | 作用 |
| --- | --- |
| `.\auto-switch-key.ps1 -Current` | 查看当前 key 详细信息（名称、额度、模型消费明细） |
| `.\auto-switch-key.ps1 -Name osr-demo_1` | 查询指定 key 详细信息 |
| `.\auto-switch-key.ps1 -Name -ALL` | 一次性查看所有 key 的完整详情（并行） |
...
3. 识别当前环境变量 `ANTHROPIC_AUTH_TOKEN` 对应的 key
